...
class NaoqiLocomotionControl(NaoqiNode):
    def __init__(self):
        ...
        self.moveIsActiveSrv = rospy.Service("move_is_active", Trigger, self.handleMoveIsActive)
    ...
    def handleGetMoveConfig(self, req):
        try:
            ...
            if req.move_config_name.data == 0:
                config_name = "Max"
            elif req.move_config_name.data == 1:
                config_name = "Min"
            elif req.move_config_name.data == 2:
                config_name = "Default"
    ...
    def handleMoveIsActive(self, req):
        try:
            res = TriggerResponse()
            res.success = self.motionProxy.moveIsActive()
            return  res
        except RuntimeError, e:
            rospy.logerr("Exception caught:\n%s", e)
            return None
