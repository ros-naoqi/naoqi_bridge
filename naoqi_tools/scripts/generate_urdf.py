...
#TODO Get motor information from documentation and generate transmission tags automatically
...
def REP120_compatibility():
    ...
    if NAME=='romeo':
        ...
        'gaze', 'fixed', None, ur.Pose((OFFSETS_DICO['CameraLeftEyeOffsetX'],0,OFFSETS_DICO['CameraLeftEyeOffsetZ']),(0,0,0))))
        MESH_VERSION = ""

    elif NAME=="nao":
        robot.add_link(ur.Link('gaze'))
        robot.add_joint(ur.Joint('gaze_joint', 'Head',
        'gaze', 'fixed', None, ur.Pose((OFFSETS_DICO['CameraTopV4OffsetX'],0,OFFSETS_DICO['CameraTopV4OffsetZ']),(0,0,0))))
        if VERSION == 'V32':
...
#TODO create all transmission elements : Cannot get them from the lib for now
    """
    This function should instanciate all transmission tags :
    - hardware interface
    - mechanical reduction ratio for each motor
    - joint and actuator to which the transmission tag reference
    for now naoTransmission.xacro has been done by hand based on the work of Konstantinos Chatzilygeroudis in his nao_dcm project https://github.com/costashatz/nao_dcm
    """
    return
...
#TODO instantiate plugins according to sensors present in input urdf
    """
    This function should instanciate all gazebo tags :
        - sensor plugins
        - mimic joints plugins
        - ros_control plugin
        - disable_links plugins
        - gazebo reference for every link (ok)
    for now naoGazebo.xacro has been done by hand based on the work of Konstantinos Chatzilygeroudis in his nao_dcm project https://github.com/costashatz/nao_dcm
    """
    return
