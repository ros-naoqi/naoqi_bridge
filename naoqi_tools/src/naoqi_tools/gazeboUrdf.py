...
class Gazebo(object):
    ...
    """
    ATTRIBUTE_NAMES=['material','turnGravityOff','dampingFactor','maxVel',\
                ...
                for attribute in gaze.ATTRIBUTE_NAMES:
                    if child.localName == attribute :
                        if attribute == 'sensor':
                            ...
                            setattr(gaze,attribute,str(child.childNodes[0].nodeValue))
    ...
    def parse(cls,node, verbose=True):
        for child in children(node):
            if child.localName == 'cameraName':
                plug = CameraPlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'bumperTopicName':
                plug = BumperPlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'leftFrontJoint':
                plug = DrivePlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'odometryTopic':
                plug = OdometryPlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'height':
                plug = VideoPlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'mimicJoint':
                plug = MimicJointPlugin(node.getAttribute('name'),node.getAttribute('filename'))
                break
            elif child.localName == 'robotSimType':
                plug = SimuPlugin(node.getAttribute('name'),node.getAttribute('filename'))
            else:
                plug = ImuLaserPlugin()
        for child in children(node):
            if child.localName in cls.ATTRIBUTE_NAMES:
                setattr(plug,child.localName,str(child.childNodes[0].nodeValue))
        return plug
...
class SimuPlugin(BasePlugin):
    """
    Define the simulation plugin used to initialize simulation parameters in gazebo
    plugin
        robotNamespace
        robotSimType
    """
    ...
    def __init__(self,name=None,filename=None,robot_namespace=None,robot_sim_type=None):
        super(SimuPlugin, self).__init__(name,filename,{'robotNamespace': robot_namespace,
                                                        'robotSimType': robot_sim_type})
#        self.filename = filename

class MimicJointPlugin(BasePlugin):
    """
    Defines the plugin used to define mimic joints (joint not commanded directly by an actuator but depending to another joint position)
    plugin
        joint
        mimicjoint
        multiplier
    ...
    def __init__(self,name=None,filename=None,joint=None,mimic_joint=None,multiplier=None,offset=None):
        super(MimicJointPlugin, self).__init__(name,filename,{'joint': joint,
...
class BumperPlugin(BasePlugin):
    ...
    plugin
        ...
        updateRate
    """
    ATTRIBUTE_NAMES = ['bumperTopicName', 'frameName','alwaysOn','updateRate']
    def __init__(self,name=None,filename=None,bumper_topic_name=None,frame_name=None,always_on=None,update_rate=None):
        super(BumperPlugin, self).__init__(name,filename,{'bumperTopicName': bumper_topic_name,
...
class ImuLaserPlugin(BasePlugin):
    ...
    plugin
        ...
        bodyName
    """
    ATTRIBUTE_NAMES = ['robotNamespace', 'topicName','gaussianNoise','xyzOffset','rpyOffset','alwaysOn','updateRate','frameName', 'hokuyoMinIntensity','frameId','accelGaussianNoise','headingGaussianNoise','rateGaussianNoise','bodyName']
    def __init__(self,name=None,filename=None,robot_namespace=None,topic_name=None,gaussian_noise=None,xyz_offset=None,rpy_offset=None, always_on=None,update_rate=None, hokuyo_min_intensity=None, frame_id=None,accel_gaussian_noise=None,heading_gaussian_noise=None,rate_gaussian_noise=None,body_name=None):
        super(ImuLaserPlugin, self).__init__(name,filename,{'robotNamespace': robot_namespace,
...
class VideoPlugin(BasePlugin):
    ...
    plugin
        ...
        updateRate
    """
    ATTRIBUTE_NAMES = ['topicName','height','width','alwaysOn','updateRate']
    def __init__(self,name=None,filename=None,topic_name=None,height=None,width=None, always_on=None,update_rate=None):
        super(VideoPlugin, self).__init__(name,filename,{'topicName': topic_name,
                                                         'height': height,
                                                         'width': width,
                                                         'alwaysOn':always_on,
                                                         'updateRate':update_rate})
...
class OdometryPlugin(BasePlugin):
    ...
    plugin
        ...
        update_rate
    """
    ATTRIBUTE_NAMES = ['commandTopic','odometryTopic','odometryFrame','odometryRate','robotBaseFrame','alwaysOn','updateRate']
    def __init__(self,name=None,filename=None,command_topic=None,odometry_topic=None,odometry_frame=None,odometry_rate=None,robot_base_frame=None, always_on=None,update_rate=None):
        super(OdometryPlugin, self).__init__(name,filename,{'commandTopic': command_topic,
...
class CameraSensor(object):
    """
    Class defining the camera sensor. This can instantiate specific plugin according to the type of camera (RGB,Stereo,Depth...)
    camera
        horizontal_fov
        image
        clip
...
class Clip(object):
    """
    Class defining the clip element used in sensors (for range or beaming sensors)
    structure:
        clip
            near
            far
